...
class SecurePairingAgent(PairingAgentBase):
    ...
    async def RequestPinCode(self, device: "o") -> "s":
        ...
        try:
            device_info = await self._get_device_info(device)
            self.logger.info(f"Device requesting PIN: {device_info['name']} ({device_info['address']})")
        ...
        await self._notify_event({
            "type": "request_pin_code",
            "device_path": device,
            "pin": pin,
            "message": f"Device requests PIN: {pin}"
        })
    ...
    async def RequestPasskey(self, device: "o") -> "u":
        ...
        try:
            device_info = await self._get_device_info(device)
            self.logger.info(f"Device requesting passkey: {device_info['name']} ({device_info['address']})")
        except Exception as e:
            self.logger.warning(f"Could not get device info: {e}")
        ...
        await self._notify_event({
            ...
            "device_path": device,
            "passkey": passkey,
            "message": f"Device requests passkey: {passkey:06d}"
        })
